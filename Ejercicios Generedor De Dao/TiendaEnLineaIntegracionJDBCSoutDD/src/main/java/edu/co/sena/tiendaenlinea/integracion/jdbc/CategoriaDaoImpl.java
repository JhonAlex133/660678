...
calls to this DAO, otherwise a new Connection will be allocated for each operation.
		...
		try {
			...
			if (dto.isIdCategoriaModified()) {
				if (dto.getIdCategoria() != null) {
					stmt.setInt( index++, dto.getIdCategoria().intValue() );
				} else {
			...
			if (dto.isActivaModified()) {
				if (dto.getActiva() != null) {
					stmt.setShort( index++, dto.getActiva().shortValue() );
				} else {
			...
			if (dto.isParienteModified()) {
				if (dto.getPariente() != null) {
					stmt.setInt( index++, dto.getPariente().intValue() );
				} else {
					stmt.setNull(index++, java.sql.Types.INTEGER);
				}
	...
	public void update(CategoriaPk pk, Categoria dto) throws CategoriaDaoException
	{
		...
		try {
			...
			if (pk.getIdCategoria() != null) {
				stmt.setInt( index++, pk.getIdCategoria().intValue() );
			} else {
				stmt.setNull(index++, java.sql.Types.INTEGER);
			}
	...
	public void delete(CategoriaPk pk) throws CategoriaDaoException
	{
		...
		try {
			...
			if (pk.getIdCategoria() != null) {
				stmt.setInt( 1, pk.getIdCategoria().intValue() );
			} else {
	 ...
	 */
	public Categoria findByPrimaryKey(Integer idCategoria) throws CategoriaDaoException
	{
		Categoria ret[] = findByDynamicSelect( SQL_SELECT + " WHERE idCategoria = ?", new Object[] { idCategoria } );
	 ...
	 * Returns all rows from the categoria table that match the criteria 'idCategoria = :idCategoria'.
	 */
	public Categoria[] findWhereIdCategoriaEquals(Integer idCategoria) throws CategoriaDaoException
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE idCategoria = ? ORDER BY idCategoria", new Object[] { idCategoria } );
	}
	...
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE activa = ? ORDER BY activa", new Object[] { activa } );
	}

	/** 
	 * Returns all rows from the categoria table that match the criteria 'pariente = :pariente'.
	 */
	public Categoria[] findWhereParienteEquals(Integer pariente) throws CategoriaDaoException
	{
		return findByDynamicSelect( SQL_SELECT + " WHERE pariente = ? ORDER BY pariente", new Object[] { pariente } );
	}
	...
	{
		dto.setIdCategoria( new Integer( rs.getInt(COLUMN_ID_CATEGORIA) ) );
		dto.setNombreCategoria( rs.getString( COLUMN_NOMBRE_CATEGORIA ) );
		dto.setActiva( new Short( rs.getShort(COLUMN_ACTIVA) ) );
		dto.setPariente( new Integer( rs.getInt(COLUMN_PARIENTE) ) );
		reset(dto);
	}
